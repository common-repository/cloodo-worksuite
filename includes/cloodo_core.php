...
$core_version = 10; /* change core version when update */

add_action('init', function() {
	if( ! function_exists('get_plugin_data') ){
		require_once( ABSPATH . 'wp-admin/includes/plugin.php' );
	}
	defined('CLOODO_BASE_API') or define('CLOODO_BASE_API','https://erp-amz.cloodo.com/v4/');
					...
					if ($am['parent_slug'] == null && (!$am['token_require'] || ($am['token_require'] && $token))){ 
						add_menu_page(						
							...
							function() use ($am){
								...
								?> 
								<div id="cloodo_admin_default" style="display:block;">
	...
	if (isset($_GET['token'])) {
		...
		$response = json_decode(wp_remote_get(CLOODO_BASE_API . 'accounts/details', $author_token)['body']);
